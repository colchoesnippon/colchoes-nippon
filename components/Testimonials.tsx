  ...
  return (
    <section className="py-24 bg-black border-t border-white/5 overflow-hidden relative">
      <div className="container mx-auto px-6">
        ...
        {/* Header with Google Proof */}
        <div className="flex flex-col items-center text-center mb-16">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="inline-flex items-center gap-3 bg-zinc-900/80 backdrop-blur-md border border-white/10 px-6 py-3 rounded-full mb-8 shadow-lg shadow-white/5"
          >
            <GoogleLogo />
            <div className="flex flex-col items-start">
              ...
              <span className="text-xs text-gray-400">Baseado em +500 avaliações</span>
            </div>
          </motion.div>
          ...
          <p className="text-gray-400 text-lg max-w-2xl">
            Junte-se a milhares de clientes que transformaram suas noites de sono com a tecnologia Nippon.
          </p>
        </div>
        ...
        <div className="relative group">
           ...
           <button 
            onClick={() => scroll('left')}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-zinc-800/50 backdrop-blur-md border border-white/10 p-3 rounded-full text-white opacity-0 group-hover:opacity-100 hover:bg-zinc-700 transition-all duration-300 hidden md:block -ml-6"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          >
            {reviews.map((review, index) => (
              <motion.div
                ...
                className="min-w-[85vw] md:min-w-[400px] snap-center bg-zinc-900/30 border border-white/10 p-8 rounded-3xl backdrop-blur-sm flex flex-col relative hover:bg-zinc-900/50 transition-colors"
              >
                ...
                <div className="mt-auto pt-4 border-t border-white/5">
                  <div>
                    <h4 className="text-white font-medium flex items-center gap-2">
                      {review.name} 
                      <span title="Compra Verificada" className="text-green-500"><CheckCircle2 size={14} /></span>
                    </h4>
                    <span className="text-xs text-gray-500">{review.location} • {review.date}</span>
                  </div>
                </div>
              </motion.div>
            ))}
           ...
           <button 
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 z-20 bg-zinc-800/50 backdrop-blur-md border border-white/10 p-3 rounded-full text-white opacity-0 group-hover:opacity-100 hover:bg-zinc-700 transition-all duration-300 hidden md:block -mr-6"
          >
